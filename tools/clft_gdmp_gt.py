...
def _fill_holes_binary_robust(mask01: np.ndarray) -> np.ndarray:
    """
    Robust hole fill for 0/1 mask.
    Padding을 두르고 floodFill을 수행해서 seed(0,0)이 항상 background가 되게 함.
    """
    m = (mask01 > 0).astype(np.uint8)  # 0/1
    ...
    cv2.floodFill(flood, ffmask, (0, 0), 128)  # border-connected background mark

    bg_border = (flood == 128).astype(np.uint8)
    holes = ((inv == 255).astype(np.uint8) & (1 - bg_border)).astype(np.uint8)

    filled = ((m_pad == 1).astype(np.uint8) | holes).astype(np.uint8)
    filled = filled[1:-1, 1:-1]  # crop padding
    return filled


def _refine_binary_mask(
    mask01: np.ndarray,
    close_k: int = 7,
    close_iters: int = 1,
    ...
    max_abs_frac: float = 0.35,
) -> np.ndarray:
    """
    mask01 (H,W) 0/1 uint8 -> refined 0/1 uint8
    close + hole fill + tiny CC removal + area explosion guard
    """
    ...
    if do_fill_holes:
        m = _fill_holes_binary_robust(m)

    if min_cc_area and min_cc_area > 0:
    ...
    # safety: area explosion -> rollback
    ref_area = int(m.sum())
...
def build_gdmp_gt_from_edt(
    ...
    # refinement
    refine: bool = False,
    refine_close_k: int = 7,
    refine_close_iters: int = 1,
    refine_fill_holes: bool = True,
    refine_min_cc_area: int = 30,
    refine_max_growth: float = 3.0,
    refine_max_abs_frac: float = 0.35,
):
    """
    ...
    for b in range(B):
        ...
        for cid in class_ids:
            ...
            if cls_mask_valid.any():
                ...
                if refine:
                    cls_u8 = _refine_binary_mask(
                        cls_u8,
                        close_k=refine_close_k,
                        close_iters=refine_close_iters,
                        do_fill_holes=refine_fill_holes,
                        min_cc_area=refine_min_cc_area,
                        max_growth=refine_max_growth,
                        max_abs_frac=refine_max_abs_frac,
                    )
...
class GDMPBuilderGT:
    """
    Semantic-only GDMP GT 빌더 (EDT 기반).
    ...
    def __init__(
        ...
        self.refine_close_k = int(refine_close_k)
        self.refine_close_iters = int(refine_close_iters)
        self.refine_fill_holes = bool(refine_fill_holes)
        self.refine_min_cc_area = int(refine_min_cc_area)
        self.refine_max_growth = float(refine_max_growth)
        self.refine_max_abs_frac = float(refine_max_abs_frac)

    def __call__(self, anno: torch.Tensor):
        M_gt, weights, valid = build_gdmp_gt_from_edt(
            anno=anno,
            ...
            # refinement
            refine=self.refine,
            refine_close_k=self.refine_close_k,
            refine_close_iters=self.refine_close_iters,
            refine_fill_holes=self.refine_fill_holes,
            refine_min_cc_area=self.refine_min_cc_area,
            refine_max_growth=self.refine_max_growth,
            refine_max_abs_frac=self.refine_max_abs_frac,
        )
        return {
            "M_gt": M_gt,        # (B,C,H,W)
